...
const play = (level = 1) => {
  ...
  const brickColCount = level === 1 ? 4 : 6;
  const bricks = [];
  ...
  const createBricks = () => {
    for (let i = 0; i < brickRowCount; i++) {
      ...
      for (let j = 0; j < brickColCount; j++) {
        const x =
          i * (brickProprs.w + brickProprs.padding) + brickProprs.offsetX;
        const y =
          j * (brickProprs.h + brickProprs.padding) + brickProprs.offsetY;
